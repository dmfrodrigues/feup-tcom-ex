...
\begin{minipage}{0.30\textwidth}
	\begin{center}
		\begin{tikzpicture}[->,>=stealth',node distance=1.7cm,initial text=$ $,]
			...
			\draw   (q1)	edge[bend left=10, right] node{$1$} (q3)
					(q3)	edge[bend left=10, left ] node{$1$} (q1)
					(q1)	edge[bend left= 7, above] node{$0$} (q2)
					(q2)	edge[bend left= 7, below] node{$0$} (q1)
					(q3)	edge[bend left=10, left ] node{$0$} (q2)
					(q2)	edge[bend left=10, right] node{$1$} (q3)
					;
		\end{tikzpicture}
	\end{center}
